# main.py - 전체 파이프라인 실행 스크립트 (프레임 저장 경로 일관화)

import os
import argparse
import subprocess
import json
from datetime import datetime

def run_command(command_list, desc):
    print(f"\n🚀 {desc}...")
    result = subprocess.run(command_list)
    if result.returncode != 0:
        print(f"❌ {desc} 실패!")
        exit(1)
    print(f"✅ {desc} 완료!")

def download_youtube_video(url, output_dir="data/raw_videos"):
    os.makedirs(output_dir, exist_ok=True)
    video_id = url.strip("/").split("/")[-1]
    output_path = os.path.join(output_dir, f"{video_id}.mp4")

    result = subprocess.run(["yt-dlp", "-e", url], capture_output=True, text=True)
    title = result.stdout.strip()
    meta_path = f"data/results/{video_id}_meta.json"
    os.makedirs("data/results", exist_ok=True)
    with open(meta_path, "w", encoding="utf-8") as f:
        json.dump({"title": title}, f, ensure_ascii=False)

    cmd = ["yt-dlp", "-f", "mp4", "-o", output_path, url]
    run_command(cmd, f"YouTube 영상 다운로드 ({video_id})")
    return output_path, video_id

def main(video_path, video_id):
    start_time = datetime.now()
    print(f"⏱️ 시작 시간: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    
    # ✅ 중첩 방지: 프레임은 data/frames/<video_id> 에 바로 저장되도록 함
    frame_dir = f"data/frames/{video_id}"
    vision_output = f"data/results/{video_id}_vision.json"
    audio_output = f"data/results/{video_id}_audio.json"
    ocr_output = f"data/results/{video_id}_ocr.json"
    sentiment_output = f"data/results/{video_id}_final_sentiment.json"
    wordcloud_output = f"data/results/{video_id}_wordcloud.png"

    # 1. 프레임 추출 (출력 경로 명확하게 지정)
    run_command([
        "python", "src/extract_frames.py",
        "--video", video_path,
        "--output_dir", frame_dir
    ], "프레임 추출")

    # 2. 대표 프레임 선택 (단일 디렉토리 기준으로 변경)
    scene_frames = sorted([f for f in os.listdir(frame_dir) if f.endswith('.jpg')])
    if not scene_frames:
        print(f"❌ 대표 프레임을 찾을 수 없습니다: {frame_dir}")
        exit(1)
    first_image_path = os.path.join(frame_dir, scene_frames[0])

    # 3. 시각 정보 요약
    run_command([
        "python", "src/extract_vision.py",
        "--image", frame_dir,
        "--output", vision_output
    ], "시각 정보 요약")

    # 4. 오디오 감정 분석
    run_command([
        "python", "src/extract_audio.py",
        "--input", video_path,
        "--output", audio_output
    ], "오디오 정보 요약")

    # 5. OCR 텍스트 감정 분석
    run_command([
        "python", "src/extract_ocr.py",
        "--input_dir", frame_dir,
        "--output", ocr_output
    ], "OCR 정보 요약")

    # 6. 감정 통합 분석
    run_command([
        "python", "src/fuse_emotions.py",
        "--vision", vision_output,
        "--audio", audio_output,
        "--ocr", ocr_output,
        "--output", sentiment_output
    ], "감정 통합 분석")

    # 7. 감정 결과 시각화
    run_command([
        "python", "src/visualize_results.py",
        "--input", sentiment_output,
        "--output", wordcloud_output
    ], "감정 결과 시각화")
    
    end_time = datetime.now()
    duration = end_time - start_time
    print(f"\n🕒 종료 시간: {end_time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"⏳ 총 소요 시간: {str(duration)}")

    print("\n✨ 모든 분석이 완료되었습니다!")
    print("👉 streamlit run dashboard.py 로 대시보드를 실행해보세요!")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CASS 멀티모달 감정 분석 파이프라인")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--video", type=str, help="로컬 영상 경로 (.mp4)")
    group.add_argument("--url", type=str, help="YouTube Shorts URL")
    args = parser.parse_args()

    if args.url:
        video_file, video_id = download_youtube_video(args.url)
    else:
        video_file = args.video
        video_id = os.path.splitext(os.path.basename(video_file))[0]

    main(video_file, video_id)
